...
async def generate_brief(topic: str, post_days: int = 90) -> str:
    ...
    (meta_results, history_ctx, competitor_ctx,
     demographics, trend_ctx, reach_by_type) = await asyncio.gather(
        meta_task,
        fetch_history_context(),
        fetch_competitor_context(),
        meta_insights.fetch_audience_demographics(),
        fetch_live_trends(),
        meta_insights.fetch_reach_by_content_type(kpi_since, until),
        return_exceptions=True,
    )
    ...
    if meta_ok:
        ...
        if rbt:
            lines += ["", "## Reach by content type (last 30 days)"]
            for ct, vals in sorted(rbt.items(), key=lambda x: -x[1]["total"]):
                ...
                if non_fol:
                    ...
                    lines.append(f"• {label}: {total:,} reach ({pct:.0f}% non-followers)")
                else:
                    lines.append(f"• {label}: {total:,} reach")
