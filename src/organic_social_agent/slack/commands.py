...
async def _run_mentions_command(channel_id: str) -> None:
    """Post the last 24h of tagged posts to the channel — runs in the background."""
    from datetime import datetime, UTC

    from organic_social_agent.reporting.meta_insights import fetch_tagged_posts
    from organic_social_agent.slack.scheduler import build_mentions_blocks, filter_last_24h

    try:
        today = datetime.now(UTC).date()
        cutoff = datetime(today.year, today.month, today.day, tzinfo=UTC) - timedelta(days=1)
        recent = filter_last_24h(await fetch_tagged_posts(), cutoff)
        await _post_message(
            channel_id,
            f"Daily tags — {len(recent)} post(s) tagged @heyharper in the last 24h",
            blocks=build_mentions_blocks(recent, today),
        )
    except Exception as exc:
        logger.error("/social mentions failed: {!r}", exc)
        await _post_message(channel_id, "Sorry, fetching tags failed. Try again in a moment.")


async def _handle_competitors(rest: str) -> dict:
    """List / add / remove competitor IG handles in the runtime config table."""
    from organic_social_agent.db_config import get_config, set_config
    ...
    if action == "add" and target:
        if target in handles:
            return {"response_type": "ephemeral", "text": f"@{target} is already tracked."}
        handles.append(target)
        await set_config("competitor_ig_handles", ",".join(handles))
    elif action == "remove" and target:
        if target not in handles:
            return {"response_type": "ephemeral", "text": f"@{target} isn't in the list."}
        ...
        return {
            "response_type": "ephemeral",
            "text": (
                "*Usage:* `/social competitors [list | add <handle> | remove <handle>]`\n"
                "• `/social competitors add gorjana`\n"
                "• `/social competitors remove mejuri`"
            ),
        }

    listing = "\n".join(f"• @{h}" for h in handles) or "_(none tracked)_"
    return {
        "response_type": "ephemeral",
        "text": f"*Tracked competitor accounts*\n{listing}",
    }
...
async def handle_social(form: dict) -> dict:
    ...
    if sub == "mentions":
        asyncio.create_task(_run_mentions_command(form.get("channel_id", "")))
        return {
            "response_type": "ephemeral",
            "text": "Pulling tags from the last 24 hours… :label:",
        }

    if sub == "competitors":
        return await _handle_competitors(rest)
    ...
    return {
        ...
        "text": (
            ...
            "• `/social mentions` — posts that tagged @heyharper in the last 24h\n"
            "• `/social competitors [list | add <handle> | remove <handle>]` — tracked competitors\n"
