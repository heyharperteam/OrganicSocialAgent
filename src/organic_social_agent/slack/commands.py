...
_DAYS_RE = re.compile(r"--days\s+(\d+)", re.IGNORECASE)
        ...
        if not rest:
            return {
                ...
                "text": (
                    ...
                    "Example: `/social brief Sub Box reveal for back-to-school season`\n"
                    "Example: `/social brief --days 14 waterproof reel`"
                ),
            }
        channel_id = form.get("channel_id", "")
        user_id = form.get("user_id", "")
        days_match = _DAYS_RE.search(rest)
        post_days = int(days_match.group(1)) if days_match else 90
        topic = _DAYS_RE.sub("", rest).strip()
        asyncio.create_task(_run_brief_command(channel_id, user_id, topic, post_days))
        days_note = f" (last {post_days} days)" if days_match else ""
        return {
            "response_type": "ephemeral",
            "text": f"Generating brief for *{topic}*{days_note}… should be ready in ~20 seconds. :pencil:",
        }
