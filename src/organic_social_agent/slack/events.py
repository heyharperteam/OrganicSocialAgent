...
async def _build_context(question: str, post_days: int = 90) -> str:
    ...
    if is_strategy:
        ...
        meta_results, stories, reach_by_type = await asyncio.gather(
            meta_task,
            fetch_recent_stories(),
            fetch_reach_by_content_type(kpi_since, until),
            return_exceptions=True,
    ...
    # Active stories (always included — show even when none/unavailable)
    ...
    if isinstance(stories, list) and stories:
        ...
        lines.append("(no active stories at this time)")
    else:
        lines.append("(stories unavailable — API error)")
