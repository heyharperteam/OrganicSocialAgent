...
_WINDOW_RE = re.compile(
    r"(?:last|past)\s+(\d+)\s+(hour|day|week|month)s?"
    r"|last\s+(week|month)"
    r"|this\s+(week|month)"
...
def _parse_window(text: str) -> int | None:
    """Extract an explicit day count from natural language, or return None.

    Hours are converted to days (rounded up, minimum 1).
    Examples: 'last 24 hours' → 1, 'last 2 weeks' → 14, 'past 60 days' → 60.
    """
    ...
    if n_str and unit:
        if unit.lower() == "hour":
            return max(1, math.ceil(int(n_str) / 24))
    ...
    if ago_n and ago_unit:
        if ago_unit.lower() == "hour":
            return max(1, math.ceil(int(ago_n) / 24))
...
async def _build_context(question: str, post_days: int = 90) -> str:
    ...
    from organic_social_agent.reporting.meta_insights import (
        fetch_recent_stories, fetch_reach_by_content_type,
    )
    from organic_social_agent.reporting.history import load_story_snapshots
    ...
    if is_strategy:
        ...
        (meta_results, history_ctx, competitor_ctx, live_stories,
         story_archive, demographics, trend_ctx, reach_by_type) = await asyncio.gather(
            meta_task,
            fetch_history_context(),
            fetch_competitor_context(),
            fetch_recent_stories(),
            load_story_snapshots(post_since, until),
            fetch_audience_demographics(),
            fetch_live_trends(),
            fetch_reach_by_content_type(kpi_since, until),
            return_exceptions=True,
        )
    else:
        meta_results, live_stories, story_archive, reach_by_type = await asyncio.gather(
            meta_task,
            fetch_recent_stories(),
            load_story_snapshots(post_since, until),
            fetch_reach_by_content_type(kpi_since, until),
    ...
    rbt = reach_by_type if isinstance(reach_by_type, dict) else {}
    TYPE_LABEL = {"REEL": "Reels", "POST": "Feed posts", "CAROUSEL_CONTAINER": "Carousels",
                  "STORY": "Stories", "AD": "Ads"}
    if rbt:
        rbt_lines = ["", "## Reach by content type (last 30 days)"]
        for ct, vals in sorted(rbt.items(), key=lambda x: -x[1]["total"]):
            label = TYPE_LABEL.get(ct, ct.title())
            total = vals["total"]
            non_fol = vals["non_follower"]
            if non_fol:
                pct = 100.0 * non_fol / total if total else 0.0
                rbt_lines.append(f"• {label}: {total:,} reach ({pct:.0f}% non-followers)")
            else:
                rbt_lines.append(f"• {label}: {total:,} reach")
        kpi_header = kpi_header + rbt_lines
    ...
    for p in top_posts:
        follower_note = (
            ...
            if p.reach_follower or p.reach_non_follower
            else " (follower breakdown N/A)"
        )
        if p.product_type == "REELS":
            watch = f"{p.avg_watch_time_ms / 1000:.1f}s" if p.avg_watch_time_ms else "N/A"
            if p.completion_rate > 0:
                comp = f"{p.completion_rate * 100:.0f}%"
            elif p.completion_rate == -1.0:
                comp = "N/A (views missing)"
            else:
                comp = "N/A"
            reel_note = f" · avg watch {watch} · {comp} completion"
        else:
            reel_note = ""
        lines.append(
    ...
    # Stories: DB archive for the window + live 24h (deduplicated by media_id)
    archived = story_archive if isinstance(story_archive, list) else []
    live = live_stories if isinstance(live_stories, list) else []
    ...
    for s in live:
        if s.media_id not in seen_ids:
            all_stories.append({
                "media_id": s.media_id, "caption": s.caption, "permalink": s.permalink,
                "snapshot_date": None,
                "impressions": s.impressions, "reach": s.reach,
                "taps_forward": s.taps_forward, "taps_back": s.taps_back,
                "exits": s.exits, "replies": s.story_replies,
            })

    lines += ["", f"## Stories ({post_since} to {until})"]
    if all_stories:
        for row in all_stories:
            date_tag = f" [{row['snapshot_date']}]" if row.get("snapshot_date") else " [live]"
            cap = (row.get("caption") or "")[:80].strip() or "(no caption)"
            lines.append(
                f'• "{cap}"{date_tag}'
                f" — {row['impressions']:,} impressions · {row['reach']:,} reach"
                f" · {row['taps_back']} tap-backs · {row['exits']} exits"
                + (f" · {row['replies']} replies" if row.get("replies") else "")
            )
    else:
        lines.append("(no story data for this window — archive builds daily)")
    ...
    if is_strategy:
        ...
        lines += ["", "## Audience demographics (current snapshot)"]
        if isinstance(demographics, AudienceDemographics) and demographics.gender_age:
            ...
            ga_str = ", ".join(f"{k} ({v / total_ga * 100:.0f}%)" for k, v in top_ga)
            ...
            lines.append("(demographics unavailable — API error or no data returned)")
    if is_strategy:
        lines += ["", "## Current Instagram/TikTok trends (live)"]
        lines.append(trend_ctx if isinstance(trend_ctx, str) and trend_ctx
                     else "(trend search unavailable)")
...
async def _handle_mention(event: dict) -> None:
    ...
    try:
        ...
        # Interval validated — acknowledge before the slow API + Claude calls
