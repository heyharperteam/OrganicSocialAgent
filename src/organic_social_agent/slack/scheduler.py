...
async def _run_daily_mentions() -> None:
    ...
    if not (s.slack_bot_token and s.slack_channel_id):
    ...
    n = len(recent)
    TYPE_LABEL = {"IMAGE": "Photo", "VIDEO": "Video", "CAROUSEL_ALBUM": "Carousel"}

    blocks: list[dict] = [
        {"type": "header",
         "text": {"type": "plain_text", "text": f"🏷️ Daily Tags — {date_str}", "emoji": True}},
    ]
    ...
    if not recent:
        ...
        for p in recent:
            type_label = TYPE_LABEL.get(p.media_type.upper(), p.media_type or "Post")
    ...
    try:
        client.chat_postMessage(
            ...
            text=f"Daily tags — {n} post(s) tagged @heyharper in the last 24h",
            blocks=blocks,
        )
        logger.info("daily mentions alert posted: {} tagged post(s)", n)
