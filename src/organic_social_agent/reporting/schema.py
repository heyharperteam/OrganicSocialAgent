...
class PostPerformance(BaseModel):
    ...
    # Post-level follower vs non-follower reach breakdown
    reach_follower: int = 0
    reach_non_follower: int = 0

    # Reel-only retention metrics (REELS product_type)
    avg_watch_time_ms: int = 0      # ig_reels_avg_watch_time (milliseconds)
    completion_rate: float = 0.0    # derived: 1 - reels_skip_rate (0–1)

    # Story-only metrics (STORY product_type)
    impressions: int = 0
    taps_forward: int = 0
    taps_back: int = 0
    exits: int = 0
...
class AudienceDemographics(BaseModel):
    """Snapshot of follower demographics from account-level lifetime insights."""

    gender_age: dict[str, int] = {}     # e.g. {"F.25-34": 1500, "M.18-24": 200}
    top_countries: dict[str, int] = {}  # e.g. {"US": 8200, "CA": 400}
    top_cities: dict[str, int] = {}     # e.g. {"Los Angeles, CA": 600}


class TaggedPost(BaseModel):
    """A post by another account in which the IG Business account was tagged.

    Sourced from /{ig-user-id}/tags. Only photo/video tags are available via
    polling — @mentions in captions or comments require webhook subscription.
    """

    media_id: str
    username: str = ""      # the account that posted it
    caption: str = ""
    media_type: str = ""    # IMAGE / VIDEO / CAROUSEL_ALBUM
    permalink: str = ""
    timestamp: str = ""     # ISO-8601 from Meta API
    like_count: int = 0

    def short_caption(self, n: int = 120) -> str:
        cap = " ".join(self.caption.split())
        return cap if len(cap) <= n else cap[: n - 1].rstrip() + "…"
