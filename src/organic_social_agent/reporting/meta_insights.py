...
_REEL_METRICS = "ig_reels_avg_watch_time,ig_reels_video_view_full_plays_rate"
...
def _parse_insights_values(data: dict | None) -> dict[str, float]:
    """Extract {metric_name: value} from a standard insights response."""
    out: dict[str, float] = {}
...
async def _fetch_post_metrics(client: httpx.AsyncClient, m: dict) -> PostPerformance:
    ...
    tasks = [
        ...
        _get(client, f"/{m['id']}/insights", {"metric": "reach", "breakdown": "follower_type"}),
    ]
    if is_reel:
    ...
    if len(results) > 2 and not isinstance(results[2], Exception):
        ...
        if is_reel:
            ...
            post.completion_rate = extras.get("ig_reels_video_view_full_plays_rate", 0.0)
        elif is_story:
