...
from organic_social_agent.reporting.schema import AudienceDemographics, Kpis, PostPerformance, TaggedPost
...
_REEL_METRICS = "ig_reels_avg_watch_time,reels_skip_rate"
...
async def fetch_reach_by_content_type(
    since: date, until: date
) -> dict[str, dict[str, int]]:
    """Reach broken down by content type, with follower/non-follower split where available.

    Tries combined breakdown (media_product_type + follow_type) first; falls back to
    type-only if the API doesn't support combining them.

    Returns {content_type: {"total": N, "non_follower": N, "follower": N}}
    e.g. {"REEL": {"total": 5000000, "non_follower": 4800000, "follower": 200000}}
    Content types: REEL, POST, CAROUSEL_CONTAINER, STORY, AD.
    """
    s, u = _epoch(since), _epoch(until)

    def _parse(total_value: dict | None) -> dict[str, dict[str, int]]:
        out: dict[str, dict[str, int]] = {}
        for bd in (total_value or {}).get("breakdowns", []) or []:
            keys = [str(k).lower() for k in (bd.get("dimension_keys") or [])]
            for res in bd.get("results", []) or []:
                dims = res.get("dimension_values") or []
                val = int(res.get("value") or 0)
                ct = ft = None
                for i, key in enumerate(keys):
                    if key == "media_product_type" and i < len(dims):
                        ct = str(dims[i]).upper()
                    elif key == "follow_type" and i < len(dims):
                        ft = str(dims[i]).upper()
                if ct is None:
                    # single-dimension breakdown — dims[0] is the content type
                    if dims:
                        ct = str(dims[0]).upper()
                if ct:
                    if ct not in out:
                        out[ct] = {"total": 0, "non_follower": 0, "follower": 0}
                    out[ct]["total"] += val
                    if ft == "NON_FOLLOWER":
                        out[ct]["non_follower"] += val
                    elif ft == "FOLLOWER":
                        out[ct]["follower"] += val
        return out

    async with httpx.AsyncClient(timeout=30.0) as client:
        # Try combined breakdown first
        tv = await _account_total(client, "reach", s, u, breakdown="media_product_type,follow_type")
        result = _parse(tv)
        if not result:
            # Fall back to type-only (no follower split)
            tv = await _account_total(client, "reach", s, u, breakdown="media_product_type")
            result = _parse(tv)

    if result:
        logger.info("reach by content type: {}", {k: v["total"] for k, v in result.items()})
    else:
        logger.warning("reach by content type: no data returned")
    return result
...
async def fetch_audience_demographics() -> AudienceDemographics:
    ...
    Uses the v21+ metric names: engaged_audience_demographics and
    reached_audience_demographics with a breakdown parameter. The response uses
    total_value.breakdowns[].results[] (not the old values[].value dict format).
    Requires the read_insights permission on the token.
    """

    def _extract_breakdown(resp: dict | None) -> dict[str, int]:
        """Parse {dimension_label: count} from a breakdowns-style demographics response."""
        ...
        for bd in (tv.get("breakdowns") or []):
            ...
            for res in (bd.get("results") or []):
                ...
                out[key] = int(res.get("value") or 0)
            if out:
                return out
        return {}
    ...
    demo = AudienceDemographics(
        gender_age=_extract_breakdown(gender_age if not isinstance(gender_age, Exception) else None),
        top_countries=_extract_breakdown(countries if not isinstance(countries, Exception) else None),
        top_cities=_extract_breakdown(cities if not isinstance(cities, Exception) else None),
    )
...
async def fetch_tagged_posts() -> list[TaggedPost]:
    """Posts by other accounts where the IG Business account was photo/video tagged.

    Uses /{ig-user-id}/tags — returns up to 50 most-recent tagged posts.
    API limitation: @mentions in captions or comments are not available via
    polling; only photo/video/Reel tags are returned here.
    """
    async with httpx.AsyncClient(timeout=30.0) as client:
        data = await _get(
            client,
            f"/{settings.meta_ig_user_id}/tags",
            {"fields": "id,username,caption,media_type,permalink,timestamp,like_count", "limit": 50},
        )
    ...
    logger.info("fetched {} tagged post(s)", len(posts))
    return posts
